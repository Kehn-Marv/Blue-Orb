...
// Enhanced function to save keys securely using IndexedDB
export function saveKeysSecurely({ nsec, npub, role = 'student', username = '', bio = '' }) {
  ...
  // Store in IndexedDB for better security (future enhancement)
  if (typeof window !== 'undefined' && 'indexedDB' in window) {
    try {
      const request = indexedDB.open('nostr_keys', 1);
      request.onupgradeneeded = (event) => {
        const db = event.target.result;
        if (!db.objectStoreNames.contains('keys')) {
          db.createObjectStore('keys', { keyPath: 'npub' });
        }
      };
      request.onsuccess = (event) => {
        ...
        store.put({ npub, nsec, role, username, bio, createdAt: Date.now() });
      };
    } catch (e) {
