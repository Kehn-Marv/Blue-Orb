...
async function init() {
  ...
  await client.execute('CREATE TABLE IF NOT EXISTS exams (id TEXT PRIMARY KEY, title TEXT, subject TEXT, year TEXT, description TEXT, fileCID TEXT, createdAt TEXT);');
  await client.execute('CREATE TABLE IF NOT EXISTS materials (id TEXT PRIMARY KEY, title TEXT, description TEXT, author TEXT, category TEXT, thumbnailCID TEXT, fileCID TEXT, createdAt TEXT);');
  ...
  try {
    await client.execute('ALTER TABLE exams ADD COLUMN description TEXT;');
    console.log('[db] Added description column to exams table');
  } catch (e) {
    // Column might already exist, ignore error
    if (!e.message.includes('duplicate column name')) {
      console.log('[db] Exams description column already exists or error:', e.message);
    }
  }
  
  try {
    await client.execute('ALTER TABLE materials ADD COLUMN description TEXT;');
    console.log('[db] Added description column to materials table');
  } catch (e) {
    if (!e.message.includes('duplicate column name')) {
      console.log('[db] Materials description column already exists or error:', e.message);
    }
  }
  
  try {
    await client.execute('ALTER TABLE materials ADD COLUMN thumbnailCID TEXT;');
    console.log('[db] Added thumbnailCID column to materials table');
  } catch (e) {
    if (!e.message.includes('duplicate column name')) {
      console.log('[db] Materials thumbnailCID column already exists or error:', e.message);
    }
  }
  
  console.log('[db] Turso tables ensured and migrated.');
}
...
/* Exams */
async function getAll_exams(){ const r = await client.execute('SELECT id, title, subject, year, description, fileCID, createdAt FROM exams ORDER BY createdAt DESC;'); return r.rows; }
async function getById_exams(id){ const r = await client.execute('SELECT id, title, subject, year, description, fileCID, createdAt FROM exams WHERE id = ?;', [id]); return r.rows[0] || null; }
async function create_exams(payload){ const id = payload.id || _makeId(); const createdAt = new Date().toISOString(); await client.execute('INSERT INTO exams (id, title, subject, year, description, fileCID, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?);', [id, payload.title||'', payload.subject||'', payload.year||'', payload.description||'', payload.fileCID||'', createdAt]); return { id, title: payload.title, subject: payload.subject, year: payload.year, description: payload.description, fileCID: payload.fileCID, createdAt }; }
async function update_exams(id, updates){ const existing = await getById_exams(id); if(!existing) return null; const newObj = Object.assign({}, existing, updates, { updatedAt: new Date().toISOString() }); await client.execute('UPDATE exams SET title = ?, subject = ?, year = ?, description = ?, fileCID = ? WHERE id = ?;', [newObj.title||'', newObj.subject||'', newObj.year||'', newObj.description||'', newObj.fileCID||'', id]); return Object.assign({ id }, newObj); }
async function delete_exams(id){ await client.execute('DELETE FROM exams WHERE id = ?;', [id]); return true; }

/* Materials */
async function getAll_materials(){ const r = await client.execute('SELECT id, title, description, author, category, thumbnailCID, fileCID, createdAt FROM materials ORDER BY createdAt DESC;'); return r.rows; }
async function getById_materials(id){ const r = await client.execute('SELECT id, title, description, author, category, thumbnailCID, fileCID, createdAt FROM materials WHERE id = ?;', [id]); return r.rows[0] || null; }
async function create_materials(payload){ const id = payload.id || _makeId(); const createdAt = new Date().toISOString(); await client.execute('INSERT INTO materials (id, title, description, author, category, thumbnailCID, fileCID, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?);', [id, payload.title||'', payload.description||'', payload.author||'', payload.category||'', payload.thumbnailCID||'', payload.fileCID||'', createdAt]); return { id, title: payload.title, description: payload.description, author: payload.author, category: payload.category, thumbnailCID: payload.thumbnailCID, fileCID: payload.fileCID, createdAt }; }
async function update_materials(id, updates){ const existing = await getById_materials(id); if(!existing) return null; const newObj = Object.assign({}, existing, updates, { updatedAt: new Date().toISOString() }); await client.execute('UPDATE materials SET title = ?, description = ?, author = ?, category = ?, thumbnailCID = ?, fileCID = ? WHERE id = ?;', [newObj.title||'', newObj.description||'', newObj.author||'', newObj.category||'', newObj.thumbnailCID||'', newObj.fileCID||'', id]); return Object.assign({ id }, newObj); }
